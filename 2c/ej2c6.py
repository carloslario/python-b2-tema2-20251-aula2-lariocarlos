...
def perform_pca(data: pd.DataFrame, n_components: int) -> PCA:
    scaler = StandardScaler() #estandariza los datos para que los campos tengan una media de 0 y una desviacion estandar de 1.
    data_scaled = scaler.fit_transform(data) #data_scaled tiene el df estandarizado
    pca = PCA(n_components=n_components) #cuantos componentes se van a emplear
    pca.fit(data_scaled)
    return pca

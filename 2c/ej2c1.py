...
def select_rows_and_columns(df: pd.DataFrame, columns: Union[List[str], List[int]], rows: Optional[slice] = None) -> pd.DataFrame:
    if rows is None:
...
def select_rows_with_conditions(df: pd.DataFrame, conditions: Union[str, List[str]]) -> pd.DataFrame:
